...
> ⚠️ **查找替换悬浮栏已按 VS Code 重做外观（B77）**：折叠态约 34px、扁平图标按钮、
> 输入框内嵌开关、两档悬停色，详见 `src/styles/global.css` 的「悬浮查找 / 替换栏」一节。
> 补拍时请以 B77 之后的版本为准 —— 此前任何查找栏截图都已过期。
> 现有四张截图（`main.png` 等）**都没有打开查找栏**（它是浮层、默认关闭），故本次不受影响。
