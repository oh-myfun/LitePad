...
钢笔沿对角线反放——**笔头朝左下、笔尾在右上**。
（不排除背景时白纸在浅色底上会糊成一片，故纸面保留极淡描边兜底。）

B43 调整（用户反馈）：钢笔缩短、笔尖不再顶到纸面角落（落在文档中间靠下），
并**去掉折角文档自身的投影**（原先折角处与下方两个圆角处能看见阴影）。
几何参数集中在 `B_*` 常量里，改图标只需要动那几个数。
...
# --------------------------------------------------------------------------- #
# 几何参数集中在此。钢笔方向固定为 135° 对角线，所以改 B_PEN_LENGTH 时
# 笔尖与笔尾会**同时**沿对角线移动：
#     笔尖 = 中心 + (长度/2)·(-0.707, +0.707)
#     笔尾 = 中心 - 同量
# 想让笔尖落在某个位置，先由「笔尖 - (长度/2)·(-0.707,+0.707)」反算中心。
B_CARD = (0.232, 0.150, 0.684, 0.830)  # 文档外框 x0,y0,x1,y1（归一化）
B_PEN_LENGTH = 0.62                    # 钢笔总长（B43：0.78 → 0.62，原偏长）
B_PEN_CENTER = (0.650, 0.400)          # 钢笔中心
B_PEN_ANGLE = -135                     # 笔头朝左下角、笔尾在右上角
B_CARD_SHADOW = False                  # B43：去掉文档投影（折角处与下方两圆角处）

# 上面三项的推算结果，改参数后请用 scripts/_icon_probe.py 复核：
#   笔尖 (0.431, 0.619) —— 文档中心是 (0.458, 0.490)，即「中间靠下」，不再顶到角上
#   笔尾 (0.869, 0.181)
...
def variant_b(s):
    ...
    钢笔反放：笔头朝左下、笔尾在右上（局部 +x 是笔尖方向，故旋转 -135°）。
    """
    base = Image.new("RGBA", (s, s), (0, 0, 0, 0))
    bx0, by0, bx1, by1 = B_CARD
    ...
    _place_tool(base, _tool(s, "pen", length=B_PEN_LENGTH), s, B_PEN_CENTER, B_PEN_ANGLE)
    return base
