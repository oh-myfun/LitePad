...
// 恒真 ⇒ 守卫咬得住。
//
// 与 `scripts/reverse-verify-*.cjs` 的关系（两者互补，不重复）：
//   · 脚本那一层是**改真实源码**再跑子进程里的用例，覆盖面含「静态契约」；代价是要临时
//     改写文件，被硬杀会留补丁（`pitfalls/0095`），过滤器写错还会假绿。
//   · 本文件**不改任何源码**，也没有过滤器 —— 随 `npm test` 一起跑，零副作用。
//   · 反过来，替身只能证明「这么写会坏」，证明不了「源码真的没这么写」—— 那一半仍由
//     各域测试里的静态契约守着（`filedrop.test.ts` / `tabdnd.test.ts`）。
//
