...
fn collect_files(dir: &Path, out: &mut Vec<PathBuf>, budget: &mut usize) {
    if *budget == 0 {
        return;
    }
...
fn search_one_file(
    path: &Path,
    matcher: &(dyn Fn(&str) -> Option<usize> + Send + Sync),
    max_per_file: usize,
) -> Vec<SearchHit> {
    let Ok(content) = fs::read(path) else {
        return Vec::new();
    };
    if doc::is_probably_binary(&content) {
        return Vec::new();
    }
...
pub async fn search_files(
    ...
    let max = max_results.unwrap_or(200).min(1000);
    if query.is_empty() {
        return Ok(Vec::new());
    }
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let mut budget = SEARCH_MAX_FILES;
        collect_files(&root, &mut files, &mut budget);

        let hits = std::sync::Mutex::new(Vec::<SearchHit>::new());
        let threads = std::thread::available_parallelism()
            .map(|n| n.get())
            .unwrap_or(4);
        let chunk = (files.len() / threads).max(1);

        std::thread::scope(|scope| {
            for chunk_files in files.chunks(chunk.max(1)) {
                let hits_ref = &hits;
                let m = matcher.clone();
                scope.spawn(move || {
                    for path in chunk_files {
                        if hits_ref.lock().map(|h| h.len()).unwrap_or(max) >= max {
                            return;
                        }
                        for hit in search_one_file(path, m.as_ref(), 20) {
                            let mut guard = match hits_ref.lock() {
                                Ok(g) => g,
                                Err(_) => return,
                            };
                            if guard.len() >= max {
        ...
        let mut result = hits.into_inner().unwrap_or_default();
