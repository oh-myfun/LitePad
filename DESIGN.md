...
├─ shell/menu.ts  全局弹出菜单（状态栏编码/行尾、导出、菜单栏下拉；支持嵌套子菜单）
├─ theme/         亮/暗/跟随系统
└─ styles/        CSS 变量驱动的双主题
```
...
## B42 菜单重组：设置菜单 + 可编辑快捷键

菜单栏从「文件/编辑/查看/帮助」改为 **文件 / 编辑 / 查看 / 设置 / 帮助**，
把散落的偏好与快捷键说明收拢成一个入口。

- **「查看」瘦身**：移除 主题三态、预览行距三档、大纲宽度三档、分屏三项
  （左右/上下/移除）。保留 源码⇄预览、大纲、折叠展开、缩放、自动换行、状态栏。
- **「文件」瘦身**：移除「新建文件默认行尾…」「新建文件默认编码…」。
- **「帮助」只留关于**：快捷键说明移入设置。
- **新增「设置」菜单**：
  - `首选项 ▸` —— 二级子菜单，扁平单选（用分隔线分组），收齐被移出的全部预设值：
    主题 / 预览行距 / 大纲宽度 / 新建默认行尾 / 新建默认编码。行尾与编码候选在启动时
    预取（`loadPreferenceOptions`），子菜单因此可以同步渲染，不再就地弹第三层列表。
  - `快捷键…` —— 打开快捷键对话框，**可浏览也可编辑**。
- **分屏只留快捷键**（用户选择）：菜单里不再有分屏项，保留
  `Alt+Shift+→` 左右分屏、`Alt+Shift+↓` 上下分屏，并新增 `Alt+Shift+←` 移除分屏
  —— 否则「能拆不能合」，功能会失联。
- **子菜单能力**（menu.ts）：`MenuItem.submenu` 支持数组或惰性函数；各层菜单平铺挂 body
  （不嵌套，避免被父级 overflow 裁切），用 `chain`/`created` 双数组管理层级与回收；
  hover 同级切换会收起旧分支，Escape / 点击外部 / resize 收起整棵；菜单自身滚动时收起子菜单。

### 快捷键注册表（`src/shell/keymap.ts`）

原先快捷键散落在 3 个 window keydown 监听里、且用 `e.key.toLowerCase()` 手写分支，
既不能改键，也无法与菜单提示保持一致。B42 收归为一张命令表：

- `CommandDef{id,label,group,keys[],editable,force}`；`keys` 支持多条默认键位
  （如标签切换的 `Ctrl+Tab` / `Ctrl+PgDn`），用户一旦自定义就只保留自定义那条。
- 解析**以 `KeyboardEvent.code` 为主**：`Ctrl+Shift+[` 的 `e.key` 是 `{`，
  只看 `key` 会把不同键位混为一谈。
- 末段键名**宽容**：接受任意命名键。因为 `bindingFromEvent` 会派生出
  `numpad0` / `intlbackslash` 这类键名，若只认白名单，录进来的键位会在下次启动被
  sanitize 掉，表现为「改了键却失效」。
- 裸字母/数字/符号**不可绑定**（`isUsableBinding`），否则会抢走正常输入。
- 分发按 overrides 对象身份缓存扁平索引，避免每次按键遍历命令表。
- `force: true` 仅给 F5：事件即便已被上层消化也必须接管（WebView2 默认 F5 刷新页面）。
- 覆盖表持久化到 `Settings.keymap`（Rust 侧 `HashMap<String,String>`，
  `#[serde(default)]` 保证旧配置文件前向兼容）；空串表示显式解绑。

### 键位调整

- **大纲**此前没有快捷键 → 新增 `Ctrl+Shift+O`。
- **折叠展开**从 CM `foldKeymap` 收归应用层，顺带修正了一处**文档错误**：
  `Ctrl+Shift+[ / ]` 在 CM 里是「折叠/展开光标处代码块」，并非此前菜单提示的
  「折叠全部/展开全部」。现在按 CM 约定登记为两组：
  `Ctrl+Shift+[ / ]` 光标处、`Ctrl+Alt+[ / ]` 全部。editor.ts 不再挂 `foldKeymap`，
  改由 `foldCodeOperation` / `foldOperation` 驱动，菜单右侧提示实时取自注册表。

### 快捷键对话框（`keymapdialog.ts`）

搜索框过滤（命令名 / 分组 / 键位）、分组列表、点击键位按钮进入录制态
（Esc 取消，Backspace/Delete 解绑）、冲突当场拦下并提示占用者、
「恢复全部默认」、改动即时持久化（无需保存按钮）。
只读项（CM/系统内置、菜单助记符、查找栏内部键）只展示不可改。
录制监听挂在 **document 捕获阶段**而非按钮上——按钮未必拿到焦点，挂按钮会「点了没反应」；
对话框打开期间全局快捷键整体让路（否则录制 Ctrl+N 会顺手新建一个标签）。
