...
- 📦 **可交付的 exe 只有一个**（09-23 用户当场报「127.0.0.1 拒绝连接」）：只有 `tauri build`（`build-all.sh` 第 4 步）覆盖写出的 `target/release/litepad.exe` 与 `bundle/nsis/*.exe` 能交付。第 3 步 `cargo build --release` 的产物是 **dev 模式**（tauri 里 `dev = !custom-protocol`，而该 feature 只在 `tauri build` 时由 CLI 注入）→ 会去连 `devUrl` 显示「拒绝连接」。另：`dist/assets` 为空壳时打包**照样 exit 0**，产物却是一片空白（exe 3.76MB vs 正常 8.12MB）—— 交付前点数 `dist/assets`。MSVC 下**别手工设 `LIB`/`INCLUDE`/`TMP`**（MSYS 会改写，反致 LNK1181/LNK1104），默认 env 直接跑即成功 → `pitfalls/0098-release-artifact-traps.md`。
- 🖼️ **本沙箱拍不到应用界面**（09-23 对照实测）：BitBlt 抓窗口只能得到 `tauri.conf.json` 的 `backgroundColor` 底色，与「已安装的 0.10.x 可用版本」表现**完全一致**（同字节数空白图）。⇒ 判断产物好坏要看**产物硬证据**（PE 导入表 / 嵌入资源 / 体积 / `cargo:dev`），别把「截图空白」当回归。
...
⚠️ **开发机只编 release**（09-22 用户清掉 `target/debug`，13G→1.7G 仅 release，已固化）：`scripts/build-all.sh:37` 与 `.githooks/pre-push:90` 的 `cargo build`/`cargo test` 已加 `--release`（不再再生 debug）；`build-all.sh:45` 的 `debug/litepad.exe` 清单已删。`scripts/dev.sh` 的 `tauri dev` 设计上必为 debug、无法改——想彻底避免 debug 只能弃用 `tauri dev` 改用 `tauri build`（开发热重载时例外）。
