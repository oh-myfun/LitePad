...
- **B62（0917）双击复位「整组居中」+ 顺带修掉「点一下就拆散联动」**：
  ① `dblclick` 里**先改比例、后求联动集合** → `centerOf` 读实时几何，本条已挪到 50%、
  联动条还在 30%，差几百 px ≫ 2px 容差 → 集合恒空 → 只有点中的那条居中。
  **修：先把集合取出来，再统一改比例**（凡「先读几何、再据此行动」都适用）。
  ② `onUp` 原先无条件 `commit(pctFor(指针))` —— 命中区 7px 宽，**一次纯点击就能把比例推走
  约 1%（400px 容器≈4px）> 2px 容差**，点一下两条对齐的线就散伙，之后拖谁都不联动
  （「联动时灵时不灵」的真凶）。**修：记 `moved`，只有真收到 `mousemove` 才回写**
  （VS Code 的 sash 同样是「没 move 就不改尺寸」）。
  ⚠️⚠️ **测试教训（第二次）**：两个 bug 都被「**常量 rect 桩**」掩盖 —— 桩必须随 inline
  `flexBasis` 实时变化（见 `stubVerticalSash()`）。已反向验证：各自还原 → 对应用例失败。
  测试 350 vitest + 22 cargo；依据见 `docs/split-view-plan.md` 第九节。
- ⚠️ **B60–B62 观感/交互待桌面环境确认**：1px 静息线是否偏细（VS Code 也是 1px）、4px 激活线宽、
  角手柄骑线 + `all-scroll` 是否好抓、浅蓝落点（无描边）在深色下是否够醒目；
  **联动**（拖一条两条一起走 / 悬停预告 / 双击整组居中）需真机 2×2 验证一次
