...
> **本文件是地图，不是细节。** 会话只在需要某域不变量时才读对应 `ref/<domain>.md`；
> 某次踩坑根因读 `adr/<NNNN>-<slug>.md`。不要在会话开头整篇读此文件。
> 路由总表见 `MEMORY.md`；逐条背景见 `2026-09-*.md`。

## 域清单（TOC）
| 域 | 一句话 | 详情落点 |
|---|---|---|
| 文档/实例模型 | 同文件多面板同源，ChangeSet 广播 | 内联 §1（待迁 `ref/model`） |
| 置脏判据 | `docChanged`≠内容变，比字符串 | 内联 §2 |
| 布局树/分割条 | path 语义、最大化只改比例 | 内联 §3 |
| 拖拽体系 | 指针编排、选择菜单判据、影像 | 内联 §4 |
| 视图刷新红线 | mousedown 链上禁 DOM 重建 | 内联 §5 |
| 样式与主题 | CSS 变量、hidden 陷阱 | 内联 §6 |
| 功能架构落点 | 查找/大纲/设置/快捷键/菜单/标签栏/分屏/提示/保存/图标 | 内联 §7 |
| 通用教训 | IPC DTO 命名、cargo build 误区、闪一下、CM 抢键 | 内联 §8 |
| 多窗口 | 卫星窗口共享状态 | **`ref/multiwindow.md`** |

> 标注「内联」的域尚未迁出，细节仍在下方原 §；逐步迁到 `ref/` 是渐进过程，不一次性重写。
> 已迁出的 §9 见 `ref/multiwindow.md`；其关键踩坑见 `adr/0072-webview2-shared-env.md`。
...
> **已迁出到 `ref/multiwindow.md`**（含 9.1–9.4 全部不变量，含拖出/拖回与 B72 踩坑指针）。
> 关键踩坑「卫星窗口须照抄主窗口 WebView2 参数」见 `adr/0072-webview2-shared-env.md`。
> 本 § 不再内联细节，避免与 `ref/` 双源漂移。
