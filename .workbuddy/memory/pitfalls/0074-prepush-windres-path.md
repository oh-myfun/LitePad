...
- ⚠️ **跟进（09-19，B83）**：本条只解决了「探测被短路」。探测命中后还有第二个坑 ——
  把 Windows 风格目录（`C:/msys64/mingw64/bin`）**原样**写进 PATH 时，cargo 这类
  **原生子进程**依然找不到 windres，钩子打印「windres: C:/…」却报 cargo 失败。
  命中后必须 `cd … && pwd` 归一成 `/c/…`。详见 `0083-prepush-windres-posix-path.md`。
