...
1. **选落点**：按 `.workbuddy/memory/MEMORY.md` 的「路由表」放，别自己发明位置。
   速判：每次会话必守的 → `MEMORY.md`「关键红线」；某域的不变量 → `ref/`；
   可枚举的约束清单 → `rules/index.md`；踩坑根因 → `pitfalls/`；
   给贡献者看的 → 仓库 `docs/`。
   ⚠️ **MEMORY.md 是精简索引**，规则详情别堆进去（09-18 五分法重构的初衷就是给它瘦身）。
...
4. **同步「照抄型」文档**：`docs/build-env.md`（可发布命令手册）与 `ref/session-env.md`
   （沙箱专属坑）是精确命令手册，规则与它冲突时**必须改它**（下个会话就是照它抄命令的）；
   `.workbuddy/skills/**` 里已固化的老教条同理，冲突的整条改写，别只加注解。
...
> 其余红线以 `MEMORY.md`「关键红线」为唯一权威源，这里**不复制**，只列该处没有的：
...
- ⚠️ **工作区脏时绝不提交**：本项目常有并行的 Bxx 改动在飞（实测 `git status` 隔几分钟
  ...
  - ⚠️ 更精确的机制（B76 实测，`main` 因此红过一次）：**钩子跑的是「工作区」，不是「你要推的那个
    提交」**。所以只要「一部分改动进了 commit、另一部分还留在工作区」，本地门就可能全绿而 CI 红 ——
    典型例子：新功能改了两个文件（组件 + 测试）却只 `git add` 了组件，本地 vitest 用的是工作区里
    那份新测试（绿），CI 检出的是提交（旧测试 → 红）。
    ⇒ 推送前自查：`git show --stat HEAD` 与工作区预期**对得上**吗？有没有新增/改写的测试还在工作区？
  - 所以这条红线是**唯一防线**，不是洁癖；**别用 `git stash -u`**（沙箱内禁用，见 `MEMORY.md`）。
- ⚠️ 同一文件的编辑一律**串行**（并发写会互相覆盖，且两次都回报成功）。
- ⚠️ 规则若涉及删/搬既有文件：**先只读扫描出报告**，列清单 + 等显式确认才动手，绝不默认清理。
